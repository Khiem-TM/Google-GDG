...
def get_active(session: Session, food_id: UUID) -> Food | None:
    return session.scalar(select(Food).where(Food.id == food_id, Food.status == "active", Food.deleted_at.is_(None)))


def get_any(session: Session, food_id: UUID) -> Food | None:
    return session.scalar(select(Food).where(Food.id == food_id, Food.deleted_at.is_(None)))
...
def list_active(session: Session, limit: int) -> list[Food]:
    return list(
        session.scalars(
            select(Food)
            .where(Food.status == "active", Food.deleted_at.is_(None))
...
def list_servings(session: Session, food_id: UUID) -> list[FoodServing]:
    return list(
        session.scalars(
            select(FoodServing)
            .where(FoodServing.food_id == food_id, FoodServing.status == "active")
...
def list_nutrients(session: Session, food_id: UUID) -> list[tuple[FoodNutrient, NutrientDefinition]]:
    return list(
        session.execute(
            ...
            .where(FoodNutrient.food_id == food_id, NutrientDefinition.status == "active")
...
def get_serving_for_food(session: Session, food_id: UUID, serving_id: UUID) -> FoodServing | None:
    return session.scalar(
        select(FoodServing).where(
            FoodServing.id == serving_id, FoodServing.food_id == food_id, FoodServing.status == "active"
        )
    )
...
def nutrient_definitions_by_code(session: Session, codes: set[str]) -> dict[str, NutrientDefinition]:
    ...
    definitions = session.scalars(
        select(NutrientDefinition).where(NutrientDefinition.code.in_(codes), NutrientDefinition.status == "active")
    )
